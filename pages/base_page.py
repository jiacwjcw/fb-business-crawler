...
class BasePage:
    ...
    def wait_for_browser_title(self, exp_title: str, timeout=60):
        for _ in range(timeout):
            logging.debug("Wait for browser title equals to {}, now is {}".format(exp_title, self.driver.title))
            if self.driver.title == exp_title:
                break
            time.sleep(1)
        else:
            raise TimeoutError("Unable to wait for browser title equals to {}".format(exp_title))
        
    def wait_for_browser_title_by_partial(self, partial_title: str, timeout=60):
        for _ in range(timeout):
            logging.debug("Wait for browser title equals to {}, now is {}".format(partial_title, self.driver.title))
            if partial_title in self.driver.title:
                break
            time.sleep(1)
        else:
            raise TimeoutError("Unable to wait for browser title partialy equals to {}".format(partial_title))
